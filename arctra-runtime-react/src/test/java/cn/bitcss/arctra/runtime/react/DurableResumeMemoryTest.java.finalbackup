...
class DurableResumeMemoryTest {
  ...
  void completion_chatMemoryContinuity() {
    ...
    store.getCheckpoints().put("process-1", checkpoint);
  ...
  void reSuspension_preservesSessionId() {
    ...
    SuspensionCheckpoint nextCheckpoint = store.getCheckpoints().get("process-1");
  ...
  static class FakeCheckpointStore extends InMemoryCheckpointStore {
    // Expose parent's checkpoints for test verification
    @Override
    public Map<String, SuspensionCheckpoint> getCheckpoints() {
      return super.getCheckpoints();
    }
  }
   ...
   */
  static class ConflictingCheckpointStore extends InMemoryCheckpointStore {
    // Expose parent's checkpoints for test verification
    @Override
    public Map<String, SuspensionCheckpoint> getCheckpoints() {
      return super.getCheckpoints();
