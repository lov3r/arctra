...
class DurableResumeExecutionTest {
  ...
  private static class FakeCheckpointStore extends InMemoryCheckpointStore {
    // Inherits all M7 methods from InMemoryCheckpointStore
    // Expose internal storage via public getter for test verification
    public Map<String, SuspensionCheckpoint> checkpoints() {
      // Return mutable copy for test modification
      return new java.util.concurrent.ConcurrentHashMap<>(getCheckpoints());
    }
  }
  ...
  private static class CountingCheckpointStore extends InMemoryCheckpointStore {
    private final java.util.concurrent.atomic.AtomicInteger loadCount = new java.util.concurrent.atomic.AtomicInteger(0);
    // Expose parent's internal store as 'checkpoints' for test compatibility
    public final Map<String, SuspensionCheckpoint> checkpoints = getCheckpoints();

    @Override
    public Optional<SuspensionCheckpoint> load(String processId) {
      loadCount.incrementAndGet();
      return super.load(processId);
    }
